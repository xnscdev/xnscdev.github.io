...
const pmu = [1, 1.1, 1.2, 1.3, 1.4, 1.5, 1.75, 2, 2.5, 3, 4, 5, 6, 7, 8, 9, 10, 12, 14, 16, 18, 20, 24, 28, 32, 36, 40, 45, 50, 75, 100, 101, 101, 101, 101, 101];
...
function update() {
    ...
    let rpx = px;
    if (fp) {
        for (let i = 1; i < 50; i++) rpx -= lx(cp, i);
    }
    _pp.innerHTML = (rpx * 100 / (cpx - ppx)).toFixed(4);

    let nd = xnl;
    for (let i = cl + 1; i < sl; i++) nd += lx(cp, i);
